...
+----------------------------+---------------------+--------+----------+----------+-------------+-----------+-----------+--------------+-----------------+
| 模型                       | 来源                | 免费   | 可用账号 | 请求     | 成功/失败   | 首字      | 平均      | 最近状态     | 最近请求        |
+----------------------------+---------------------+--------+----------+----------+-------------+-----------+-----------+--------------+-----------------+
| deepseek-v4.1-flash        | official-cli@2.151.0 | 是     | 8        | 12       | 12/0        | 820ms     | 3.4s      | 成功         | 09-16 20:23:28  |
| glm-5.2                    | official-cli@2.151.0 | 否     | 9        | 4        | 3/1         | 1.1s      | 5.2s      | 失败:6004    | 09-16 20:11:02  |
+----------------------------+---------------------+--------+----------+----------+-------------+-----------+-----------+--------------+-----------------+
...
模型限制按“账号 + 模型”隔离：`code 6004` 只冷却触发模型，其他模型仍可调用；`code 14018` 只阻断该账号的当前收费模型。成功响应明确携带 `usage.credit=0` 时，网关学习该账号下该模型为免费模型；付费余额耗尽账号仍可优先服务已确认免费的模型，并通过 `[FreeModel]` 日志明确记录。
...
| 免费 | 是按账号+模型实测聚合：是 / 否 / 混合 / - |
| 可用账号 | 当前真正可调度该模型的账号数（已计入账号冷却、模型冷却、模型额度阻断） |
| 请求 | 客户端请求次数 |
| 成功/失败 | 计数 |
